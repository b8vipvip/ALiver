...
class StreamMetrics:
    ...
    def public_dict(self) -> dict[str, Any]:
        value = asdict(self)
        value["connected_seconds"] = round(
            max(0.0, time.time() - self.connected_at),
            3,
        )
...
def _allow_insecure() -> bool:
    return os.environ.get("ALIVER_ALLOW_INSECURE_PCM", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }
...
def _flush_avatar(avatar_session: Any) -> dict[str, int]:
    ...
    if player is not None and hasattr(player, "clear_queues"):
        try:
            player_cleared = player.clear_queues()
            cleared.update(
                {f"webrtc_{key}": value for key, value in player_cleared.items()}
            )
        except Exception:
...
def _put_pcm_frame(
    ...
    *,
    ...
    metrics.dropped_frames += _drop_oldest(
        avatar_session,
        _max_queue_frames(),
    )
    metadata = {
    ...
    avatar_session.put_audio_frame(
...
def _video_only_default() -> bool:
    return os.environ.get("ALIVER_VIDEO_ONLY", "1").strip().lower() not in {
        "0",
        "false",
        "no",
    }
    ...
    return _json(
        {
            ...
            "video_only_default": _video_only_default(),
        }
    )
    ...
    return _json(
        {
            "ok": True,
            "streams": [
                item.public_dict() for item in _registry(request.app).values()
            ],
        }
    )
...
async def pcm_websocket(request: web.Request) -> web.StreamResponse:
    ...
    try:
        ...
        if avatar_session is None:
            raise ValueError(
                f"LiveTalking session 不存在：{start.session_id}"
            )
        if start.stream_id in registry:
            raise ValueError(f"stream_id 已被占用：{start.stream_id}")
        if any(
            item.session_id == start.session_id for item in registry.values()
        ):
            raise ValueError(
                f"session 已有活动 PCM 流：{start.session_id}"
            )
        ...
        await ws.send_json(
            {
                "type": "started",
                **start.public_dict(),
                "max_queue_ms": _max_queue_frames() * FRAME_MS,
            }
        )
        ...
        async for message in ws:
            ...
            if message.type == WSMsgType.TEXT:
                ...
                if command == "ping":
                    metrics.queue_depth_frames = _queue_depth(avatar_session)
                    await ws.send_json(
                        {"type": "pong", "stream": metrics.public_dict()}
                    )
                    ...
                    await ws.send_json(
                        {"type": "ended", "stream": metrics.public_dict()}
                    )
                    ...
                    metrics.queue_depth_frames = _queue_depth(avatar_session)
                    await ws.send_json(
                        {"type": "status", "stream": metrics.public_dict()}
                    )
                else:
                    await ws.send_json(
                        {
                            "type": "error",
                            "error": f"未知控制消息：{command}",
                        }
                    )
                continue
        ...
        if not ws.closed:
            try:
                ...
                logger.exception("ALiver PCM error response failed")
            await ws.close(
                code=1008,
                message=str(exc).encode("utf-8", errors="ignore")[:120],
            )
    finally:
        ...
        if metrics is not None:
            ...
            if avatar_session is not None and hasattr(avatar_session, "notify"):
                try:
                    avatar_session.notify(
                        {
                            "status": "end",
                            "stream_id": metrics.stream_id,
                            "epoch": metrics.epoch,
...
def setup_aliver_routes(app: web.Application) -> None:
    ...
    logger.info(
        "ALiver PCM routes enabled: 16kHz mono s16le, "
        "%dms/%d-byte frames, video-only default=%s",
        FRAME_MS,
        BYTES_PER_FRAME,
        _video_only_default(),
    )
